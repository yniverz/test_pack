...
execute if entity @s[name="Yniverz"] run tellraw @a {"text":"👑 The Queen has arrived ❤","color":"light_purple","bold":true}
execute if entity @s[name="Yniverz"] run title @s title {"text":"Welcome back ❤","color":"light_purple"}

# --- Default greeting for everyone else --------------------------------
execute unless entity @s[name="Yniverz"] run tellraw @a [{"selector":"@s","color":"yellow"},{"text":" joined the game","color":"gray"}]
